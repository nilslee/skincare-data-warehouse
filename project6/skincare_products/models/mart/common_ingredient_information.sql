WITH mrt_common_ingredient_information as (
  with common_product_ingredients as (
    ...
    from
      {{ ref('Product_Ingredient') }} pi
    left join
      {{ ref('Disease_Ingredient_Treatment') }} dit
    on
  ...
  ingredient_brand_count as (
    ...
    from
      {{ ref('Brand') }} b
    left join {{ ref('Sephora_Product') }} sp on sp.brand_id = b.brand_id
    left join {{ ref('Product_Ingredient') }} pi on sp.product_id = pi.product_id
    group by
