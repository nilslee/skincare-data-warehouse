...
skin_color_sql = """select distinct affected_skin_color from dbt_skincare_products_stg.skin_disease_picture where affected_skin_color is not null order by affected_skin_color"""
...
def model(dbt, session):    
    ...
    for i, row in enumerate(rows):
        ...
        skin_colors.append(row["affected_skin_color"])

        if i > 0 and i % batch_size == 0:
            print("processing batch")
            skin_colors_str = '\n'.join(skin_colors)
            replacements = do_inference(skin_colors_str)
    ...
    skin_df = dbt.ref("tmp_skin_disease_picture_name")
    
    output_df = skin_df.na.replace(combined_replacements, "affected_skin_color")
